...
class Genvis(Vita):
    ...
    def diff_feature_fusion(self, features, alpha1=1, alpha2=1):  # (B*T,C,H,W)
        BT = list(features.values())[0].shape[0]
        T = self.num_frames if self.training else BT
        B = BT // T

        for k, f in features.items():
            f = f.resize(B, T, *f.shape[1:])  # (B,T,C,H,W)
            diff_features = []
            for i in range(f.shape[1]):
                # indices = list(range(f.shape[1]))
                # indices.pop(i)

                start_ind = max(0, i - self.num_frames)
                end_ind = min(f.shape[1], i + 1 + self.num_frames)

                diff_features.append(torch.cat(
                    [f[:, start_ind:i, ...] - f[:, i, ...][:, None],
                     f[:, i, ...][:, None] - f[:, i + 1:end_ind, ...]],
                    dim=1))

            diff_features = torch.stack(diff_features, dim=1).reshape(BT, -1, *f.shape[
                                                                               -2:])  # (B,T,T-1,C,H,W) -> (B*T,(T-1)*C,H,W)
            features[k] = alpha1 * f.reshape(BT, -1, *f.shape[-2:]) + alpha2 * self.diff_conv[k](diff_features)
